...
const ButtonResult = ({
  ...
  function getComment() {
    if (correctAnswers < 4) {
      return "Не стоит огорчаться, если результат небольшой. Это отличный повод узнать цитаты более тщательно!";
      ...
      return "Безупречный результат! Вы отлично разбираетесь в этой теме!";
    }
  }
  ...
  return (
    <div className="mainDivBtnRes">
      <div {...btnMenuSection} className="divBtnRes">
        <button
          className="sn-section-item check_answ"
          tabIndex={-1}
          onClick={() => handleClick(null, "show_res")}
        >
          Результат
        </button>
      </div>
      <ModalWindow
        setActive={setModalWindowRes}
        active={modalWindowRes}
        assistant_global={handleClick}
        setModalState={setModalRes}
      >
        <div {...btnResultSection} className="divModalRes">
          <h1 style={{ display: "block" }}>{getComment()}</h1>
          <h1 style={{ display: "block" }}>Верных ответов</h1>
          <h1 style={{ display: "block" }}>{correctAnswers + " из " + 10}</h1>
          <button
            className="sn-section-item btnMenuAfterGame"
            tabIndex={-1}
            onClick={() => handleClick(null, "returnMenuAfterGame")}
          >
            Главное меню
          </button>
        </div>
